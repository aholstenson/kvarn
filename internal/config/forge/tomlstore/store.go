...
// prEntry mirrors a `pull_request` sub-table. It is a pointer everywhere it is
// used so a config without the block round-trips through Put without gaining an
// empty table.
type prEntry struct {
...
// comment a delivery posts. The keys are the sink names spelled the way the
// rest of this file spells keys.
...
// absent block and yields a zero PRContent, which contributes nothing to
// ResolveBehavior.
...
func entryToConfig(name string, e *forgeEntry) (*forgeconfig.ForgeConfig, error) {
	...
	return &forgeconfig.ForgeConfig{
		...
		PullRequest:       e.PullRequest.toContent(),
	}, nil
}
...
func configToEntry(fc *forgeconfig.ForgeConfig) (string, *forgeEntry) {
	...
	return fc.Name, &forgeEntry{
		...
		PullRequest:       prEntryFrom(fc.PullRequest),
	}
}
...
func (s *Store) Defaults(ctx context.Context) (forgeconfig.Defaults, error) {
	...
	return forgeconfig.Defaults{
		...
		Labels:            labels,
		PullRequest:       fd.Defaults.PullRequest.toContent(),
	}, nil
}
